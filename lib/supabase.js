...
// ── 초대장 참석 회신 (줄 이름: iv-<시각>-<난수>) ──────────
export async function addRsvp(reply) {
  const id = `iv-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const { error } = await supabase.from(TABLE).insert({ id, data: reply });
  if (error) throw new Error(error.message);
}
...
export async function loadRsvps() {
  ...
  for (let from = 0; ; from += 1000) {
    const { data, error } = await supabase
      .from(TABLE).select("id,data,updated_at").like("id", "iv-%")
      .order("updated_at", { ascending: false }).range(from, from + 999);
    if (error) throw new Error(error.message);
    all.push(...data);
    if (data.length < 1000) break;
  }
  return all.filter((r) => !r.data?.hidden).map((r) => ({ id: r.id, at: r.updated_at, ...r.data }));
}
